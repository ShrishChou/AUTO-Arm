...
class Processing:
    ...
    @staticmethod
    def bgr_to_intensity(bgr_image):
        # Convert the BGR image to floating-point values for higher precision
        bgr_image = bgr_image.astype(np.float64)

        # Apply the luminance formula for each pixel
        intensity = 0.2989 * bgr_image[:, :, 2] + 0.5870 * bgr_image[:, :, 1] + 0.1140 * bgr_image[:, :, 0]
    ...
    def compute_beam_parameters(image, pixel_size=0.00001, max_iterations=20, convergence_threshold=0.01):
        ...
        # To Float.64 Array of Intensities
        gray_image = Processing.bgr_to_intensity(image) 
        # To uint8 BGR Image
        image_uint8 = np.clip(image, 0, 255).astype(np.uint8) 
        contour_image = cv2.cvtColor(image_uint8, cv2.COLOR_BGR2GRAY)
        ...
        max_distance = np.max(contour_distances)
        ...
        for iteration in range(max_iterations):
            if False:
                ...
                ax.plot_surface(x_plots, y_plots, intensity.reshape(6000, 8000), cmap='viridis')
            ...
            new_centroid_x, new_centroid_y = Processing.compute_first_order_moments(new_intensity, 
                                                                                    new_x_coords, 
                                                                                    new_y_coords)
            new_sigma_x2, new_sigma_y2, new_sigma_xy = Processing.compute_second_order_moments(new_intensity, 
                                                                                   new_x_coords, 
                                                                                   new_y_coords, 
                                                                                   new_centroid_x, 
                                                                                   new_centroid_y)
            ...
            # Enforce shrinking effect: ensure new widths are smaller than old widths
            if new_d_sigma_x >= d_sigma_x or new_d_sigma_y >= d_sigma_y:
                break
            ...
            sigma_x2, sigma_y2, sigma_xy = new_sigma_x2, new_sigma_y2, new_sigma_xy
        ...
        sigma_x2 = sigma_x2 * pixel_size
        sigma_y2 = sigma_y2 * pixel_size
        sigma_xy = sigma_xy * pixel_size

        return d_sigma_x, d_sigma_y, phi_z, centroid_x, centroid_y, sigma_x2, sigma_y2, sigma_xy
...
def gaussian_beam_test():
    ...
    lambda_m = np.float64(532e-9)       # Wavelength in m

    # Derived Rayleigh range
    z_R = np.pi * (w0_m) ** 2 / lambda_m

    # Create grid of coordinates
    x = np.linspace(-image_width // 2, image_width // 2, image_width, dtype = np.float64)
    y = np.linspace(-image_height // 2, image_height // 2, image_height, dtype = np.float64)
    x, y = np.meshgrid(x, y)
    ...
    for i, z in enumerate(z_values):
        ...
        plot_frames = [5*i for i in range(21)]
        ...
        if i in plot_frames:
            ...
            circle = plt.Circle((cX_px, cY_px), beam_radius_px,
                                color='cyan', 
                                fill=False, 
                                linewidth=1, 
                                label='Beam Width')
            ...
            theory_circle = plt.Circle((image_width / 2, image_height / 2),
                                        theoretical_widths[i] / pixel_size_m,
                                        color='green',
                                        fill=False, linewidth=1, label='Theory Width')
            plt.gca().add_patch(theory_circle)

            error_width = ((beam_width_m / 2.0) / theoretical_widths[i] - 1.0) * 100.0
            z_cur = (i - 50) / 25
            print(f"At z =  {z_cur} * rayleight range")
  
            print("Predicted Width " + str(beam_width_m / 2))
            print("Theoretical Width " + str(theoretical_widths[i]))
            print("Percent Error " + str(error_width))
            # Title and legend
            plt.title(f'Beam Center and Width (Frame: {i}) and Error {error_width}%')
...
def plot_M_squared(Z, width, wavelength, theoretical_widths = None):
